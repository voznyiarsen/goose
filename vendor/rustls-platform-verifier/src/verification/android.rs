...
impl Verifier {
    ...
    fn verify_certificate(
        ...
        // On Android, the platform verifier requires a JVM `Context`, which is only
        // available inside a real Android app process. In environments like Termux
        // (where goose runs as a standalone native binary) it can never be
        // initialized, so fall back to the bundled Mozilla root store instead of
        // panicking.
        #[cfg(target_os = "android")]
        if !crate::android::is_initialized() {
            return verify_with_static_roots(
                end_entity,
                intermediates,
                server_name,
                ocsp_response,
...
/// Verify a server certificate chain using the bundled Mozilla root store
/// ([`webpki_roots`]) instead of the Android platform verifier.
///
/// This is used as a fallback when the platform verifier is not initialized
/// (e.g. Termux), where there is no JVM `Context` available.
#[cfg(target_os = "android")]
fn verify_with_static_roots(
    end_entity: &pki_types::CertificateDer<'_>,
    intermediates: &[pki_types::CertificateDer<'_>],
    server_name: &pki_types::ServerName<'_>,
    ocsp_response: Option<&[u8]>,
    ...
    let verifier = WebPkiServerVerifier::builder_with_provider(Arc::new(roots), provider.clone())
        .build()
        .map_err(|e| TlsError::General(format!("failed to build static root verifier: {e}")))?;
